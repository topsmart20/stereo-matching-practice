...
let computeNeighbours parameters i =
    ...
    [| if y > 0 then yield i - parameters.width
       ...
       if x < (parameters.width - 1) then
           yield i + 1
       if y < (parameters.height - 1) then
           yield i + parameters.width |]
...
let inline initMessages parameters (dataCosts: float32 [] []) =
    ...
    Array.init
        ...
        (fun i ->
            ...
            let arraysize =
                min ((parameters.maximumDisparity + 1)) ((Array.length dataCosts.[i]))
...
let updateMessages
    ...
    let processPixel loopIndex _loop neighbourMessageSums =
        ...
        for fp = 0 to maxD do
            for (_, (neighbourCosts: float32 [])) in p do
                neighbourMessageSums.[fp] <-
                    neighbourMessageSums.[fp]
                    + neighbourCosts.[fp]
                    + dataCosts.[i].[fp]

        for ((neighbourIdx: int), (neighbourCosts: float32 [])) in p do
            let hfp =
                Array.map2 ((-)) neighbourMessageSums neighbourCosts

            let minhfp = (Array.min hfp) + D_FH
            dtFH hfp

            let indexInNeighbour =
                Array.findIndex (fst >> ((=) i)) m.[neighbourIdx]
    ...
    Threading.Tasks.Parallel.For(startIndex, ((Array.length m) / 2), createThreadLocalArray, processPixel, ignore)
    |> ignore

    // Array.Parallel.iter (fun p ->
    Array.iter (fun p -> Array.iter (fun (_, neighbourCosts) -> normalizeCostArray neighbourCosts) p) m
...
let beliefpropagation parameters bpparameters =
    let dataCosts =
        Data.computeDataCosts parameters bpparameters.DataFunction

    let smoothnessCosts =
        Smoothness.computeSmoothnessCosts parameters bpparameters.SmoothnessFunction
