...
type CLIArguments =
    ...
    interface IArgParserTemplate with
        member s.Usage =
            ...
            | OutputDirectory _ ->
                "The directory for the file to be output to.
Note that you do NOT specify a filename - that will be automatically constructed based on the filename of the left image and the options selected"
            | Window _ ->
                "Size of the sides of the square window to be used, assuming that the selected algorithm uses windows.
The total size of the window will thus be n x n, where n is the input size here"
            | MaximumDisparity _ ->
                "The maximum disparity to search across, assuming that the selected algorithm uses a maximum"
            | Algorithm _ -> "The specific choice of stereo matching algorithm to be used"
            | NumberOfIterations _ ->
                "The number of iterations of the algorithm to perform (currently only applicable to Belief Propagation)"
...
let determineOutputFilename (results: ParseResults<CLIArguments>) =
    let algorithmString =
        results.GetResult Algorithm |> getAlgorithmString

    let leftImageName = results.GetResult LeftImage

    let leftimagewithoutextension =
        Path.GetFileNameWithoutExtension leftImageName

    let leftImageExtension = Path.GetExtension leftImageName

    let windowSize =
        if results.Contains Window then
            "_" + (results.GetResult Window |> string)
        else
            String.Empty

    let numberOfIterations =
        if results.Contains NumberOfIterations then
            "_"
            + (results.GetResult NumberOfIterations |> string)
        else
            String.Empty
...
let openImageAndConvertToGrayscaleArray (imagePath: string) =
    ...
    img.GetPixelSpan().ToArray()
    |> Array.Parallel.map (fun p -> p.R)
...
let main argv =
    ...
    let parser =
        ArgumentParser.Create<CLIArguments>(programName = "stereo matching runner", errorHandler = errorHandler)

    let results = parser.ParseCommandLine argv

    let imgWidth, imgHeight =
        results.GetResult LeftImage |> getImageSize
    ...
    let outputImageArray =
        let matchingParameters =
            { leftImage =
                  results.GetResult LeftImage
                  |> openImageAndConvertToGrayscaleArray
              rightImage =
                  results.GetResult RightImage
                  |> openImageAndConvertToGrayscaleArray
              width = imgWidth
              height = imgHeight
              totalPixels = imgHeight * imgWidth
              windowEdgeSize =
                  results.TryGetResult Window
                  |> Option.defaultValue 3
              maximumDisparity =
                  results.TryGetResult MaximumDisparity
        ...
        | DynamicProgramming ->
            let updatedMatchingParameters =
                { leftImage =
                      matchingParameters.leftImage
                      |> Array.Parallel.map byte
                  rightImage =
                      matchingParameters.rightImage
                      |> Array.Parallel.map byte
                  width = matchingParameters.width
                  height = matchingParameters.height
                  totalPixels =
                      matchingParameters.width
                      * matchingParameters.height
            ...
            (dynamicProgramming updatedMatchingParameters)
            |> Array.Parallel.map byte //(fun a -> Array.map byte a)
        | BeliefPropagation ->
            let updatedMatchingParameters =
                { leftImage =
                      matchingParameters.leftImage
            ...
            let bpparameters: BeliefPropagation.BPParameters =
                { DataFunction = (Data.FHTruncatedLinear Common.LAMBDA_FH Common.TAU_FH)
                  ...
                  Iterations =
                      results.TryGetResult NumberOfIterations
                      |> Option.defaultValue 3 }
    ...
    let outputImage =
        Image.LoadPixelData(Array.Parallel.map makeGray8 outputImageArray, imgWidth, imgHeight)

    outputImage.Mutate(fun x -> x.HistogramEqualization() |> ignore)

    let outputFilename =
        (results.GetResult OutputDirectory)
